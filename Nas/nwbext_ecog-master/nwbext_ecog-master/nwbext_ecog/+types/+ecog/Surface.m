...
methods
    ...
    function val = validate_faces(obj, val)
        val = types.util.checkDtype('faces', 'uint', val);
    ...
    function val = validate_vertices(obj, val)
        val = types.util.checkDtype('vertices', 'float', val);
        if isa(val, 'types.untyped.DataStub')
            valsz = fliplr(val.dims);
        else
            valsz = size(val);
    ...
    function refs = export(obj, fid, fullpath, refs)
        ...
        if ~isempty(obj.faces)
            ...
            elseif ~isempty(obj.faces)
                io.writeDataset(fid, [fullpath '/faces'], class(obj.faces), obj.faces, true);
            end
        else
        ...
        if ~isempty(obj.vertices)
            ...
            elseif ~isempty(obj.vertices)
                io.writeDataset(fid, [fullpath '/vertices'], class(obj.vertices), obj.vertices, true);
            end
        else
